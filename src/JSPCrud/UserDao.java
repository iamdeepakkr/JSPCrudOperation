...
public static int save(User u){
	...
	try{
		...
		PreparedStatement ps=con.prepareStatement("insert into adduser (Id,name,password,email,sex,country) values(?,?,?,?,?,?)");
		ps.setInt(1,u.getId());
...
public static int update(User u){
	...
	try{
		Connection con=getConnection();
		PreparedStatement ps=con.prepareStatement("update adduser set name=?,password=?,email=?,sex=?,country=? where id=?");
